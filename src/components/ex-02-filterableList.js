...
class FilterableList extends Component {
  ...
  render() {
    let userLists = allUsersList.filter(user => {
      if (this.state.visibleUserType === 'all') return true;
      if (this.state.visibleUserType === 'active' && user.status === 'active') return true;
      if (this.state.visibleUserType === 'inactive' && user.status === 'inactive') return true;
    });
